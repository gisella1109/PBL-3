...
<div style="max-width:100%; overflow:hidden;">
    ...
    <div style="margin-bottom:1.5rem;">
        <h1 style="font-size:2rem; font-weight:bold; color:#0067ac; margin-bottom:0.5rem;">Dashboard Member</h1>
        <p style="color:#0067ac;">Welcome back, {{ Auth::user()->display_username ?? Auth::user()->username }}!</p>
    </div>
        ...
        <div style="background: #96d3da; padding:1.25rem; border-radius:1.25rem; box-shadow:0 1px 6px rgba(0,0,0,0.08); border:1.5px solid #d0eaeb;">
            <div style="display:flex; align-items:center;">
                ...
                <div style="margin-left:0.75rem;">
                    ...
                    <p style="font-size:1.25rem; font-weight:bold; color:#fff;">{{ $meetings_count }}</p>
                </div>
            </div>
        </div>
        <div style="background: #96d3da; padding:1.25rem; border-radius:1.25rem; box-shadow:0 1px 6px rgba(0,0,0,0.08); border:1.5px solid #d0eaeb;">
            <div style="display:flex; align-items:center;">
                <div style="padding:0.5rem; background:#d0eaeb; border-radius:0.5rem;">
                    <i class="bx bx-calendar-check" style="color:#0067ac;"></i>
                </div>
                <div style="margin-left:0.75rem;">
                    <p style="font-size:0.95rem; font-weight:500; color:#0067ac;">Schedules</p>
        ...
        <div style="background: #96d3da; padding:1.25rem; border-radius:1.25rem; box-shadow:0 1px 6px rgba(0,0,0,0.08); border:1.5px solid #d0eaeb;">
            <div style="display:flex; align-items:center;">
                ...
                <div style="margin-left:0.75rem;">
                    ...
                    <p style="font-size:1.25rem; font-weight:bold; color:#fff;">{{ $teams_count }}</p>
                </div>
            </div>
        </div>
        <div style="background: #96d3da; padding:1.25rem; border-radius:1.25rem; box-shadow:0 1px 6px rgba(0,0,0,0.08); border:1.5px solid #d0eaeb;">
            <div style="display:flex; align-items:center;">
                ...
                <div style="margin-left:0.75rem;">
                    ...
                    <p style="font-size:1.25rem; font-weight:bold; color:#fff;">{{ $tasks->count() }}</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div style="background: #96d3da; padding:1.25rem; border-radius:1.25rem; box-shadow:0 1px 6px rgba(0,0,0,0.08); border:1.5px solid #d0eaeb;">
            <div style="display:flex; align-items:center;">
                ...
                <div style="margin-left:0.75rem;">
                    ...
                    <p style="font-size:1.25rem; font-weight:bold; color:#fff;">{{ $tasks->where('status', 'Selesai')->count() }}</p>
                </div>
            </div>
        </div>
        <div style="background: #96d3da; padding:1.25rem; border-radius:1.25rem; box-shadow:0 1px 6px rgba(0,0,0,0.08); border:1.5px solid #d0eaeb;">
            <div style="display:flex; align-items:center;">
                ...
                <div style="margin-left:0.75rem;">
                    ...
                    <p style="font-size:1.25rem; font-weight:bold; color:#fff;">{{ $tasks->where('status', 'Belum Selesai')->count() }}</p>
                </div>
            </div>
        </div>
        <div style="background: #96d3da; padding:1.25rem; border-radius:1.25rem; box-shadow:0 1px 6px rgba(0,0,0,0.08); border:1.5px solid #d0eaeb;">
            <div style="display:flex; align-items:center;">
                ...
                <div style="margin-left:0.75rem;">
                    ...
                    <p style="font-size:1.25rem; font-weight:bold; color:#fff;">
                        {{ $tasks->filter(function($task) { return $task->deadline < now()->format('Y-m-d') && $task->status !== 'Selesai'; })->count() }}
                    </p>
                </div>
            </div>
        </div>
    ...
    <div style="display:grid; grid-template-columns:1fr 1fr; gap:1.5rem;">
        ...
        <div style="background: #d0eaeb; border-radius:1.25rem; box-shadow:0 1px 6px rgba(0,0,0,0.08); border:1.5px solid #96d3da;">
            ...
            <div style="padding:1rem;">
                @include('components.task-table-member', ['tasks' => $tasks])
            </div>
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function() {
      $('#calendar').fullCalendar({
        events: '{{ route('member.schedules.json') }}',
      ...
      $('#accept-schedule').click(function() {
        ...
        $.post('{{ route('member.schedules.accept') }}', { schedule_id: id, action: 'accept', _token: '{{ csrf_token() }}' }, function(res) {
          $('#schedule-modal').hide();
          location.reload();
        });
      });
      ...
      $('#reject-schedule').click(function() {
        var id = $(this).data('schedule-id');
        $.post('{{ route('member.schedules.reject') }}', { schedule_id: id, action: 'reject', _token: '{{ csrf_token() }}' }, function(res) {
          $('#schedule-modal').hide();
          location.reload();
        });
      });
